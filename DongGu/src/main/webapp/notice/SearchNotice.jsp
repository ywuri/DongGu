...
<%
	...
	array = dao.searchNotice(search_type, search_word,cp,listSize);
	
	int totalCnt=dao.getNoticeCnt(search_type,search_word); // 총 게시물 수
	...
	<table id="FreeBoardTable">
		...
		<tbody>
			...
			if(array!=null){
				...
				%>
				<tr>
					<td><a href='/DongGu/notice/DetailNotice.jsp?n_num=<%=array.get(i).getN_num() %>&cp=<%=cp %>' class="FreeBoardA"><%=array.get(i).getN_title() %></a><span class="FreeBoardComment"></span></td>
					<td><%=array.get(i).getN_nickname() %></td>
					<td><%=array.get(i).getN_vcnt() %></td>
					<td><%=array.get(i).getN_date() %></td>
				</tr>
				<%
				}
			}
		...
		<tfoot>
				<tr> 
					<td colspan="4"><input class="FreeBoardButton" id="FreeBoardTableButton" type="button" value="돌아가기"  onclick="location.href='/DongGu/notice/Notice.jsp';"></td>
				</tr>
						...
						if(totalCnt!=0){
							if(userGroup>1 ){
								%>
									...
									onclick="location.href='/DongGu/notice/SearchNotice.jsp?cp=<%=(userGroup-1)*pageSize  %>&search_word=<%=search_word%>&search_type=<%=search_type%>';">
								<%
								}
							for(int i=(userGroup-1)*pageSize+1;i<=(userGroup-1)*pageSize+pageSize ;i++){
								if(cp==i){
									%>
									&nbsp;&nbsp;<a href="/DongGu/notice/SearchNotice.jsp?cp=<%=i %>&search_word=<%=search_word%>&search_type=<%=search_type%>" class="FreeBoardPageNum" style="color:red;"><%=i %></a>&nbsp;&nbsp;
								<%
								}else{
								%>
									&nbsp;&nbsp;<a href="/DongGu/notice/SearchNotice.jsp?cp=<%=i %>&search_word=<%=search_word%>&search_type=<%=search_type%>" class="FreeBoardPageNum"><%=i %></a>&nbsp;&nbsp;
								<%
								}
							...
							if( userGroup != totalPage/pageSize+(totalPage%pageSize==0?0:1)  ){
								%>
									...
									onclick="location.href='/DongGu/notice/SearchNotice.jsp?cp=<%= (userGroup+1)*pageSize- (pageSize-1)%>&search_word=<%=search_word%>&search_type=<%=search_type%>';">
								<%
								}
						}
