...
public class OtherReviewDAO {
    ...
    public OtherReviewDAO() {
        System.out.println("PidReviewDAO 객체 생성됨!");
    }
    ...
    public IdReviewDTO getReviewByNum(int r_num) {
        ...
        try {
            ...
            String sql = "SELECT r.r_num, r.r_write_id, r.r_receive_id, r.r_id_check, r.r_star, r.r_content, r.r_img, r.r_date, " +
                         "       p.p_nickname AS nickname, a.ai_name AS animal_name, " +
                         "       FLOOR(MONTHS_BETWEEN(SYSDATE, a.ai_birth) / 12) AS animal_age " +
                         "FROM review r " +
                         "LEFT JOIN petsitter p ON r.r_write_id = p.p_id " +
                         "LEFT JOIN animalinfo a ON r.r_receive_id = a.o_id " +
                         "WHERE r.r_num = ?";
