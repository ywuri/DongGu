...
public class ReviewDAO {
	...
	// 총 리뷰 수를 가져오는 메서드
    public int getReviewCnt(int rIdCheck) {
        int count = 0;
	...
	// 리뷰 목록을 페이지네이션으로 가져오는 메서드
		public ArrayList<ReviewDTO> getReviewList(int cp, int listSize, int rIdCheck) {
			...
			String sql = "SELECT * FROM ("
					...
					+ "           WHEN r.r_id_check = 1 THEN p.p_nickname " + "           ELSE 'Unknown' "
					+ "       END AS nickname, " + "       ROWNUM rnum " + "FROM review r "
					+ "LEFT JOIN owner o ON r.r_write_id = o.o_id AND r.r_id_check = 0 "
					+ "LEFT JOIN petsitter p ON r.r_write_id = p.p_id AND r.r_id_check = 1 ";
			...
			sql += "ORDER BY r.r_num DESC" + ") WHERE rnum BETWEEN ? AND ?";

			try (Connection conn = com.DongGu.db.DongGuDB.getConn(); PreparedStatement ps = conn.prepareStatement(sql)) {
				...
				try (ResultSet rs = ps.executeQuery()) {
					while (rs.next()) {
						ReviewDTO dto = new ReviewDTO(rs.getInt("r_num"), rs.getString("r_write_id"),
								rs.getInt("r_id_check"), rs.getInt("r_star"), rs.getString("r_content"),
								rs.getString("r_img"), rs.getDate("r_date"), rs.getString("nickname"));
						list.add(dto);
					}
				}
			} catch (Exception e) {
	...
	public ArrayList<ReviewDTO> MainAfterList(int rIdCheck) {
		...
		String sql = "SELECT * FROM ("
				...
				+ "       CASE " + "           WHEN r.r_id_check = 0 THEN o.o_nickname "
				+ "           WHEN r.r_id_check = 1 THEN p.p_nickname " + "           ELSE 'Unknown' "
				+ "       END AS nickname " + "FROM review r "
				+ "LEFT JOIN owner o ON r.r_write_id = o.o_id AND r.r_id_check = 0 "
				+ "LEFT JOIN petsitter p ON r.r_write_id = p.p_id AND r.r_id_check = 1 " + "WHERE r.r_id_check = ? "
				+ "ORDER BY r.r_num DESC" + ") WHERE ROWNUM <= 3"; // 최신 글 3개만 가져오기
		...
		try (Connection conn = com.DongGu.db.DongGuDB.getConn(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					...
					ReviewDTO dto = new ReviewDTO(r_num, r_write_id, r_id_check, r_star, r_content, r_img_name, r_date,
							nickname);
					arr.add(dto);
				}
			}
	...
	public ArrayList<ReviewDTO> AfterListPerson(int rIdCheck, String id, int cp, int listSize) {
		...
		String sql = "select *  " + "from  " + "(select rownum as rnum, a.* from " + "    (  "
				+ "SELECT r.r_num, r.r_write_id, r.r_id_check, r.r_star, r.r_content, r.r_img, r.r_date, "
				+ "    CASE  " + "    WHEN r.r_id_check = 0 THEN o.o_nickname "
				+ "    WHEN r.r_id_check = 1 THEN p.p_nickname  " + "    ELSE 'Unknown'  " + "    END AS nickname  "
				+ "FROM review r  " + "LEFT JOIN owner o ON r.r_write_id = o.o_id AND r.r_id_check = 0  "
				+ "LEFT JOIN petsitter p ON r.r_write_id = p.p_id AND r.r_id_check = 1  "
				+ "WHERE r.r_id_check = ? and r.r_receive_id = ? " + "ORDER BY r.r_num DESC)a  " + "    )b "
				+ "where rnum >= ? and rnum <=?";
