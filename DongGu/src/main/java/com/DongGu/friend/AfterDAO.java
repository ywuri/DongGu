...
public class AfterDAO {
   ...
   public ArrayList<AfterDTO> AfterList(int rIdCheck) {
       ...
       String sql = "SELECT r.r_num, r.r_id, r.r_id_check, r.r_star, r.r_content, r.r_img, r.r_date, " +
                    "       CASE " +
                    ...
                    "       END AS nickname " +
                    "FROM review r " +
                    "LEFT JOIN owner o ON r.r_id = o.o_id AND r.r_id_check = 0 " +
                    "LEFT JOIN petsitter p ON r.r_id = p.p_id AND r.r_id_check = 1 " +
                    "WHERE r.r_id_check = ? " +
       ...
       try (Connection conn = com.DongGu.db.DongGuDB.getConn();
           ...
           try (ResultSet rs = ps.executeQuery()) {
               while (rs.next()) {
                   ...
                   AfterDTO dto = new AfterDTO(r_num, r_id, r_id_check, r_star, r_content, r_img, r_img_name, r_date, nickname);
                   arr.add(dto);
               }
           }
       } catch (Exception e) {
